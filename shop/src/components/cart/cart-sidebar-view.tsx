...
import CartPayement from './cart-payement';
  ...
  return (
    <section className="relative flex h-full flex-col">
      ...
      <footer className="fixed bottom-0 z-10 w-full max-w-md bg-light px-6 py-5">
        <CartPayement/>
        <button
          className="flex h-12 w-full justify-between rounded bg-accent p-1 text-sm font-bold shadow-700 transition-colors hover:bg-accent-hover focus:bg-accent-hover focus:outline-0 md:h-14"
          onClick={handleCheckout}
        >
          <span className="flex h-full flex-1 items-center px-5 text-light">
            {t('text-checkout')}
          </span>
          <span className="flex h-full shrink-0 items-center rounded bg-light px-5 text-accent">
            {totalPrice}
          </span>
        </button>
      </footer>
