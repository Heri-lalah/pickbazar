...
const ShopsPage: NextPageWithLayout = () => {
  ...
  console.log('Shop data', isLoading, data)

  if (isLoading || nearShopLoading) {
    return (
      <div className="pt-6 lg:pt-10 container mx-auto px-4 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3">
        <>
          {rangeMap(limit, (i) => (
  ...
  return (
    <div className="min-h-screen bg-light">
      <div className="mx-auto flex w-full max-w-[1492px] flex-col p-8 pt-14">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3">
          {data?.map((shop) => shop?.distance && <NearShopCard
            key={shop.id}
            shop={shop}
          />)}
        </div>
